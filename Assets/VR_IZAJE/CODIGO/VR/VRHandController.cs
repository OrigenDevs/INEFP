...
public class VRHandController : MonoBehaviour
{
    ...
    void Start()
    {
        string key = "RotationOffset_" + node;
        if (PlayerPrefs.HasKey(key + "_x"))
        {
            float rx = PlayerPrefs.GetFloat(key + "_x");
            float ry = PlayerPrefs.GetFloat(key + "_y");
            float rz = PlayerPrefs.GetFloat(key + "_z");
            rotationOffset = new Vector3(rx, ry, rz);
        }
    ...
    public void AjustarRotacion(Vector3 ajuste)
    {
        rotationOffset += ajuste;
        GuardarRotacion();
    }

    public void AjustarRotacionX(float grados)
    {
        rotationOffset.x += grados;
        GuardarRotacion();
    }
    ...
    {
        string key = "RotationOffset_" + node;
        PlayerPrefs.SetFloat(key + "_x", rotationOffset.x);
        PlayerPrefs.SetFloat(key + "_y", rotationOffset.y);
        PlayerPrefs.SetFloat(key + "_z", rotationOffset.z);
        PlayerPrefs.Save();
    }
    ...
    void Update()
    {
        if (desktopOverride)
        {
            ...
            transform.rotation = desktopRotation * Quaternion.Euler(rotationOffset);
